...
- **MW-DEC-04:** Cash flow uses take-home income. Housing burden uses strictly positive gross income. Missing gross income is represented as `null` and omits burden; zero is never treated as unknown or silently substituted.
- **MW-DEC-05:** Each Priority Change compares destination utility with origin utility from documented indicators and records raw evidence, transformation, delta, weight, contribution, classification, and availability. Decision materiality requires a non-similar metric delta and `abs(delta * weight) >= materialityThreshold * 3`.
- **MW-DEC-06:** Affordability is owned by Financial Position rather than counted again as lifestyle. Weight 0 excludes a priority with a null materiality threshold. A positive weight requires a registered transformation. Missing data lowers completeness/confidence and is not imputed silently.
...
- **MW-DEC-08:** Derive Evidence Confidence as `high`, `moderate`, or `limited` from source age, geography match, uncertainty/coverage, and missing weighted evidence—not from whether the move looks favorable. No active benchmark evidence yields `limited` under `confidence.no_active_benchmark_evidence` rather than vacuous `high`.
...
- **MW-DEC-10:** Select drivers, tradeoffs, blockers, caveats, omitted dimensions, and next investigation steps deterministically. Every blocker cites its registered absolute destination metric plus contributing inputs; every material claim and next step carries stable evidence IDs.
