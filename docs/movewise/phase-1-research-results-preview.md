...
- Illustrative financial assumptions exported from `@workspace/benchmark-data`, with declared ranges on the three destination estimates.
...
- Exact condition-changing income, housing, and recurring-expense thresholds derived through canonical reevaluation.
- A What-if panel that reruns the verified decision engine as assumptions change inside their declared ranges.
...
The illustrative finances are calculation fixtures, not metro observations or a recommended budget. Their plausible ranges exist solely to exercise deterministic sensitivity behavior. The page labels them accordingly. The housing benchmark remains `context_only`; it is not substituted into those fixtures or the deterministic decision.

The What-if panel is not a forecast, simulation, or probability model. Each displayed threshold is the first cent-exact value that changes the canonical condition, proven by reevaluating the scenario at and immediately across the boundary.
...
- No Wizard, generalized simulation, multi-metro lookup, save/share flow, or public evaluation deployment is included.
