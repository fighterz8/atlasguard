...
const extractRows = (
  ...
  const expectedGeoIds = new Set<string>(
    Object.values(APPROVED_ACS_COMMUTE_SOURCE.geographies).map(
      (geography) => geography.acsGeoId,
    ),
  );
...
export const extractCommuteRowsFromOfficialTables = (
  ...
  const aggregateRows = extractRows(b08013Text, ["B08013_E001", "B08013_M001"]);
  const workerRows = extractRows(b08006Text, [
    "B08006_E001",
    "B08006_M001",
    "B08006_E017",
    "B08006_M017",
  ...
  return Object.fromEntries(
    (["origin", "destination"] as const).map((side) => {
      const geoId = APPROVED_ACS_COMMUTE_SOURCE.geographies[side].acsGeoId;
